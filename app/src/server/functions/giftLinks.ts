...
function getPlatformFromUrl(rawUrl: string): Platform | null {
  ...
  const groups = (m as RegExpMatchArray & { groups?: { idInPath?: string } })
    .groups;
  const idInPath = groups?.idInPath;
...
export const fetchProductDetails = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!platform) {
      throw new Error(
        "Invalid product URL. Must be a valid amazon.com or macys.com product link.",
      );
    }
    ...
    if (!productName) {
      throw new Error(
        `Failed to parse product name from ${platform} product page`,
      );
    }
