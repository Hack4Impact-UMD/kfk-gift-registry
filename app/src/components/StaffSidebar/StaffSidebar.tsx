  ...
  return (
    <div className="relative">
      <svg
        ...
        strokeWidth={1.5}
        stroke="currentColor"
        className={`transition-colors duration-500 ${active ? activeColor : 'text-gray-500'} group-data-[status=active]/button:${activeColor} group-hover/button:text-black ${size || "size-6"}`}
      >
        {children}
      </svg>
    </div>
...
const SidebarMenuButtonWithHovering = ({ children }: SidebarMenuButtonProps) => {
...
export function StaffSidebar() {
  ...
  return (
    <Sidebar collapsible="icon" className="duration-200">
      <SidebarTrigger
        openIcon={
          <SidebarIcon active={false} activeColor="text-black">
            <path
              strokeLinecap="round"
