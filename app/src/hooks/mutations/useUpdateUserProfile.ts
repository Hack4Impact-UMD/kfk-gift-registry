...
export function useUpdateUserProfile() {
  ...
  return useMutation({
    mutationFn: (params: { userId: string; updates: { firstName?: string; lastName?: string; phone?: string } }) =>
      updateUserProfile({ data: params }),
    onSuccess: (_data, variables) => {
      // Invalidate the specific user profile and all users list
      queryClient.invalidateQueries({ queryKey: queries.users.id(variables.userId).queryKey });
